...
# [0] Cek apakah NPM sudah terinstal, jika belum maka instal Node.js v20 LTS
if ! command -v npm &> /dev/null; then
    echo -e "${YELLOW}[0/8] NPM tidak ditemukan. Menginstal Node.js & NPM terlebih dahulu...${NC}"
...
echo -e "${YELLOW}[1/8] Menginstal PM2 secara global...${NC}"
npm install pm2 -g

# 2. Menginstal ZeroTier
echo -e "\n${YELLOW}[2/8] Menginstal ZeroTier One...${NC}"
curl -s https://install.zerotier.com | bash

# 3. Menginstal local dependencies project
echo -e "\n${YELLOW}[3/8] Menginstal dependensi lokal (npm install)...${NC}"
...
# 5. Menjalankan aplikasi dengan PM2 menggunakan npm start (production server)
echo -e "\n${YELLOW}[5/7] Menjalankan aplikasi dengan PM2 ('ndelok')...${NC}"
# Hentikan instance lama jika ada
pm2 delete ndelok 2>/dev/null || true
...
echo -e "${GREEN}Dashboard dapat diakses di: http://${HOST_IP}:1234${NC}\n"
