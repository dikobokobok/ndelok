...
export default function FileManager() {
  ...
  const { authenticatedFetch, token } = useContext(AuthContext)
  ...
  const handleNewItem = async () => {
    ...
    const itemPath = currentPath ? `${currentPath}/${newItemName.trim()}` : newItemName.trim()
    try {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden animate-in fade-in duration-300">
      {/* Top Bar */}
      <div className="flex items-center gap-3 px-5 py-3 border-b border-white/5 bg-surface-container-low shrink-0">
        <button onClick={() => navigate('/projects')} className="flex items-center gap-1.5 text-slate-400 hover:text-white transition-colors text-xs font-bold">
        ...
        <span className="text-sm font-black text-white">{project}</span>
        <span className="text-white/10">/</span>
        {/* Breadcrumb */}
        <div className="flex items-center gap-1 text-xs text-slate-400 flex-1 min-w-0">
          <button onClick={() => loadDir('')} className="hover:text-white transition-colors font-medium">root</button>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button onClick={() => {
            const fakeEntry = { name: currentPath ? currentPath.split('/').pop() : project, type: 'folder' }
            const folderPath = currentPath || ''
            const url = `/api/files/download-folder?project=${encodeURIComponent(project)}&path=${encodeURIComponent(folderPath)}`
            showToast('success', `Zipping current folder...`)
            authenticatedFetch(url).then(r => r.blob()).then(blob => {
              const name = currentPath ? currentPath.split('/').pop() : project
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* File List Panel */}
        <div className={`flex flex-col border-r border-white/5 bg-[#080d1a] overflow-hidden transition-all duration-300 ${openFile ? 'w-72 shrink-0' : 'flex-1'}`}>
          {/* Column Headers */}
        ...
        {openFile && (
          <div className="flex-1 flex flex-col overflow-hidden animate-in slide-in-from-right-4 duration-200">
            ...
            <div className="flex items-center gap-3 px-4 py-2 border-b border-white/5 bg-[#0a0f1d] shrink-0">
              <span className="material-symbols-outlined text-[14px] text-slate-400">code</span>
