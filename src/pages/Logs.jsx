...
export default function Logs() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 max-w-[1600px] mx-auto animate-in">
      {/* Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight text-on-surface">Log Stream</h2>
          <p className="text-slate-500 mt-2 text-xs sm:text-sm">Centralized activity and system event tracking.</p>
        </div>
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          <div className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition-all ${streaming ? 'bg-primary/10 text-primary' : 'bg-surface-container text-slate-400'}`}>
            <span className={`w-2 h-2 rounded-full ${streaming ? 'bg-primary animate-pulse' : 'bg-slate-500'}`} />
            {streaming ? 'Live' : 'Paused'}
          </div>
          <button onClick={() => setStreaming(v => !v)}
            className="px-3 sm:px-4 py-2 bg-surface-container-highest text-[10px] sm:text-xs font-bold rounded-xl hover:text-primary transition-colors">
            {streaming ? 'Pause' : 'Resume'}
          </button>
          <button onClick={handleExport} disabled={filtered.length === 0 || user?.role === 'viewer'}
            className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-surface-container-highest text-[10px] sm:text-xs font-bold rounded-xl hover:text-primary transition-colors disabled:opacity-50 disabled:grayscale disabled:cursor-not-allowed">
      ...
      {/* Filters */}
      <div className="flex flex-wrap items-center gap-2 sm:gap-3 bg-surface-container p-3 sm:p-4 rounded-xl">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-1 sm:gap-2">
            <span className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-500">Level</span>
            <select value={selLevel} onChange={e => setSelLevel(e.target.value)}
              className="bg-surface-container-highest border-none text-xs font-bold text-on-surface rounded-lg py-2 px-2 sm:px-3 focus:outline-none focus:ring-1 focus:ring-primary">
              {levels.map(l => <option key={l}>{l}</option>)}
            </select>
          </div>
          <div className="flex items-center gap-1 sm:gap-2">
            <span className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-500">Cat</span>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            <span className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-500">Svc</span>
      ...
      <div className="bg-surface-container-low rounded-xl overflow-hidden border border-white/5">
        ...
        <div className="hidden md:flex bg-surface-container-lowest px-5 py-2.5 items-center gap-4 border-b border-white/5">
        ...
        <div className="divide-y divide-white/5 font-telemetry text-sm">
          {filtered.length === 0 ? (
            ...
            return (
              <div key={i} className="px-3 sm:px-5 py-3 sm:py-3.5 hover:bg-surface-container transition-colors group">
                {/* Mobile layout */}
                <div className="md:hidden space-y-1.5">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <span className={`material-symbols-outlined text-[14px] ${cat.color}`}>{cat.icon}</span>
                      <span className="text-slate-400 text-[11px] font-bold truncate">{l.service}</span>
                      <span className={`text-[8px] px-1.5 py-0.5 rounded font-black uppercase ${cfg.badge}`}>{l.level}</span>
                    </div>
                    <span className="text-slate-600 text-[10px] tabular-nums flex-shrink-0">{l.time}</span>
                  </div>
                  <p className="text-on-surface-variant text-[11px] leading-relaxed">{l.msg}</p>
                  <div className="flex items-center gap-2">
                    <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold ${l.initiator === 'System' ? 'bg-slate-800 text-slate-500' : 'bg-primary/20 text-primary-fixed'}`}>
                      {l.initiator || 'System'}
                    </span>
                  </div>
                </div>
        ...
        <div className="px-3 sm:px-5 py-3 bg-surface-container-lowest border-t border-white/5 flex items-center justify-between text-[10px] sm:text-[11px] text-slate-500">
